...
> tentang prediksi crypto dengan LSTM. Sentimen dari **Fear & Greed Index**
> (alternative.me).
>
...
⚠️ **Disclaimer**: BUKAN REKOMENDASI FINANSIAL. Gunakan dengan bijak.
...
- 📊 **Data ingestion** — Yahoo Finance OHLCV (15m utama + 1h konfirmasi trend) + Fear & Greed Index harian
- 🧮 **Indikator teknikal native** — RSI, MACD, Bollinger, EMA, ATR, Stochastic, Volume Ratio diimplementasi murni pandas/numpy di `data/indicators.py` (tanpa pandas-ta, yang bermasalah di numpy ≥ 2.0)
- 🧠 **LSTM binary classifier** — memprediksi apakah harga naik ≥0,3% dalam 4 candle ke depan (1 jam)
- ⚖️ **Class-weight balancing** — label tidak seimbang ditangani otomatis saat training
- 🛡️ **Anti-leakage ketat** — `shift(1)` semua fitur, scaler fit hanya di fold training, `TimeSeriesSplit` walk-forward
- 🎯 **Gerbang konfirmasi sinyal** — minimal 2 kondisi teknikal yang mengkonfirmasi sebelum LONG/SHORT
- 💰 **Level risiko berbasis ATR** — TP1/TP2/SL dari ATR(14)
- 📁 **Log audit CSV** — semua prediksi tersimpan di `logs/predictions.csv`
- ⚡ **Cache Parquet** — TTL 15 menit, tidak refetch berlebihan
...
**Tidak perlu API key** — Yahoo Finance dan alternative.me keduanya publik.
...
### Scan semua pair → tabel sinyal terperingkat
...
Hasil lengkap → `logs/scan_{timestamp}.csv`. Cache Parquet (TTL 15 menit) dipakai
ulang sehingga scan tidak menghantam Yahoo 22× jika data masih segar.
...
Hasil lengkap → `logs/backtest_{TICKER}_{timestamp}.csv` + ringkasan hit-rate di console.
...
3. **Scaler fit per fold** — `MinMaxScaler` di-fit hanya pada fold training; data validasi hanya `.transform()`.
...
- **FGI bersifat harian** — disebar ke candle 15m, bergerak lambat.
- **Tidak ada jaminan profit.** Pasar crypto non-stasioner; model bisa terdegradasi.
